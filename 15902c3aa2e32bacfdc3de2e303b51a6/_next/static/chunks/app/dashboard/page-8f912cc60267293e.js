...
function wd(value,delta){return delta?value+" ("+delta+")":value}
...
function sMoney(v){return(v>0?"+":v<0?"-":"")+F.s7(abs(v))}
function sFp(v){return sg(v)+v+" FP"}
function sDays(v){return sg(v)+v+" "+unit(v,"day","days")}
function sumH(configs,period,key){return sumBy(configs,function(c){return F.hy(c,period)[key]||0})}
function avgH(configs,period,key){return avg(configs.map(function(c){return F.hy(c,period)[key]||0}))}
...
return{text:sFp(moved),sub:prev+" FP → "+d.totalFp+" FP",rag:dirRag(-moved)}},
...
return{text:sFp(moved),sub:total-moved+" FP → "+total+" FP",rag:dirRag(-moved)}},
metrics:[{label:"Contract type",basis:"agreed",noRollup:!0,source:"Configuration, contract-type toggle.",cell:function(c){return{text:"fp"===c.contractType?"FP-based":"Hour-based"}}},{label:"FP method",basis:"agreed",noRollup:!0,source:"Configuration, FP counting method.",cell:function(c){return{text:c.fpMethod}}},{label:"Lines of Code",basis:"to date",source:"TiCS (interface). Period view adds the lines written inside the window.",cell:function(c){return{text:c.overview.loc.toLocaleString("en-US")}},change:function(c,d,period){var v=F.hy(c,period).locAdded||0;return{text:wd(c.overview.loc.toLocaleString("en-US"),sg(v)+v.toLocaleString("en-US"))}},rollupChangeCell:function(configs,derived,period){var v=sumH(configs,period,"locAdded");return{text:wd(sumBy(configs,function(c){return c.overview.loc}).toLocaleString("en-US"),sg(v)+v.toLocaleString("en-US"))}}},{label:"Complexity Index",basis:"to date",source:"TiCS (interface). Period view adds the movement inside the window.",cell:function(c){return{text:c.overview.complexityIndex.toFixed(1)}},change:function(c,d,period){var v=F.hy(c,period).cxD||0;return{text:wd(c.overview.complexityIndex.toFixed(1),v?sNum1(v):""),rag:dirRag(-v)}}},{label:"# Developers",basis:"to date",source:"Oracle / roster. Period view adds the headcount moved inside the window.",cell:function(c){return{text:String(c.overview.developers)}},rollupCell:function(configs){return{text:String(sumBy(configs,function(c){return c.overview.developers}))}},change:function(c,d,period){var v=F.hy(c,period).devsD||0;return{text:wd(String(c.overview.developers),v?sNum(v):"")}},rollupChangeCell:function(configs,derived,period){var v=sumH(configs,period,"devsD");return{text:wd(String(sumBy(configs,function(c){return c.overview.developers})),v?sNum(v):"")}}},{label:"Total scope",basis:"forecast",emphasis:!0,source:"Derived: all counted FP, excluding deleted functions.",cell:function(c,d){return{text:d.totalFp+" FP"}},change:function(c,d,period){var v=F.hy(c,period).scopeFp||0;return{text:wd(d.totalFp+" FP",sNum(v)),rag:dirRag(-v)}},rollupChangeCell:function(configs,derived,period){var v=sumH(configs,period,"scopeFp");return{text:wd(sumBy(derived,function(d){return d.totalFp})+" FP",sNum(v)),rag:dirRag(-v)}}}]},
...
return{text:sPts(h.better),sub:gPrev!==gNow?gPrev+" → "+gNow+" ("+move+")":move,rag:dirRag(h.better)}},
...
return{text:sPts(delta),sub:score-delta+"% → "+score+"%",rag:dirRag(delta)}},
metrics:[{label:"Security (ISO 5055)",basis:"to date",emphasis:!0,source:"TiCS (interface). Period view adds the points moved inside the window.",cell:function(c){return{text:pct(c.better.security),rag:F.wq(c.better.security,c.thresholds.better)}},change:function(c,d,period){var v=Math.round(F.hy(c,period).better*BETTER_MIX.security);return{text:wd(pct(c.better.security),sNum(v)),rag:F.wq(c.better.security,c.thresholds.better)}}},{label:"Maintainability (ISO 5055)",basis:"to date",source:"TiCS (interface). Period view adds the points moved inside the window.",cell:function(c){return{text:pct(c.better.maintainability),rag:F.wq(c.better.maintainability,c.thresholds.better)}},change:function(c,d,period){var v=Math.round(F.hy(c,period).better*BETTER_MIX.maintainability);return{text:wd(pct(c.better.maintainability),sNum(v)),rag:F.wq(c.better.maintainability,c.thresholds.better)}}},{label:"Reliability (ISO 5055)",basis:"to date",source:"TiCS (interface). Period view adds the points moved inside the window.",cell:function(c){return{text:pct(c.better.reliability),rag:F.wq(c.better.reliability,c.thresholds.better)}},change:function(c,d,period){var v=Math.round(F.hy(c,period).better*BETTER_MIX.reliability);return{text:wd(pct(c.better.reliability),sNum(v)),rag:F.wq(c.better.reliability,c.thresholds.better)}}},{label:"Efficiency (ISO 5055)",basis:"to date",source:"TiCS (interface). Period view adds the points moved inside the window.",cell:function(c){return{text:pct(c.better.efficiency),rag:F.wq(c.better.efficiency,c.thresholds.better)}},change:function(c,d,period){var v=Math.round(F.hy(c,period).better*BETTER_MIX.efficiency);return{text:wd(pct(c.better.efficiency),sNum(v)),rag:F.wq(c.better.efficiency,c.thresholds.better)}}},{label:"Functional suitability",basis:"to date",source:"TiCS (interface). Period view adds the points moved inside the window.",cell:function(c){return{text:pct(c.better.functionalSuitability),rag:F.wq(c.better.functionalSuitability,c.thresholds.better)}},change:function(c,d,period){var v=Math.round(F.hy(c,period).better*BETTER_MIX.functionalSuitability);return{text:wd(pct(c.better.functionalSuitability),sNum(v)),rag:F.wq(c.better.functionalSuitability,c.thresholds.better)}}},{label:"Quality at completion",basis:"forecast",emphasis:!0,source:"Derived: average of the five ISO 5055 scores. Quality has no separate forecast model, so today's score is the forecast.",cell:function(c,d){var g=F.b9(d.betterScore);return{text:g+" · "+d.betterScore+"%",rag:F.I(g)>=F.I(c.qualityTarget)?"good":F.I(g)===F.I(c.qualityTarget)-1?"warn":"bad"}},change:function(c,d,period){var g=F.b9(d.betterScore),v=F.hy(c,period).better;return{text:wd(g+" · "+d.betterScore+"%",sNum(v)),rag:F.I(g)>=F.I(c.qualityTarget)?"good":F.I(g)===F.I(c.qualityTarget)-1?"warn":"bad"}}},{label:"Agreed quality grade",basis:"agreed",source:"Configuration, agreed quality grade.",cell:function(c){return{text:c.qualityTarget+" · "+F.gF(c.qualityTarget)+"%"}}},{label:"MTBF",basis:"to date",source:"Jira, incident tickets. Period view adds the days moved inside the window.",cell:function(c){return{text:c.better.mtbf+" days"}},change:function(c,d,period){var v=F.hy(c,period).mtbfD||0;return{text:wd(c.better.mtbf+" days",v?sNum(v):""),rag:dirRag(v)}}},{label:"MTTR",basis:"to date",source:"Jira, incident tickets. Period view adds the hours moved inside the window.",cell:function(c){return{text:c.better.mttr+" hrs"}},change:function(c,d,period){var v=F.hy(c,period).mttrD||0;return{text:wd(c.better.mttr+" hrs",v?sNum(v):""),rag:dirRag(-v)}}}]},
...
return{text:sDec(h.happier),sub:prev.toFixed(1)+" → "+d.happierScore.toFixed(1),rag:dirRag(h.happier)}},
rollupCompletion:function(configs,derived){
...
return{text:sDec(delta),sub:(Math.round(10*(score-delta))/10).toFixed(1)+" → "+score.toFixed(1),rag:dirRag(delta)}},
metrics:[{label:"Team happiness",basis:"to date",emphasis:!0,source:"Happiness survey (per sprint). Period view adds the points moved inside the window.",cell:function(c){return{text:c.happier.team.toFixed(1),rag:F.wq(c.happier.team,c.thresholds.happier)}},change:function(c,d,period){var v=Math.round(10*F.hy(c,period).happier*HAPPIER_MIX.team)/10;return{text:wd(c.happier.team.toFixed(1),sNum1(v)),rag:F.wq(c.happier.team,c.thresholds.happier)}}},{label:"Collaboration happiness",basis:"to date",source:"Happiness survey (per sprint). Period view adds the points moved inside the window.",cell:function(c){return{text:c.happier.collaboration.toFixed(1),rag:F.wq(c.happier.collaboration,c.thresholds.happier)}},change:function(c,d,period){var v=Math.round(10*F.hy(c,period).happier*HAPPIER_MIX.collaboration)/10;return{text:wd(c.happier.collaboration.toFixed(1),sNum1(v)),rag:F.wq(c.happier.collaboration,c.thresholds.happier)}}},{label:"Usage happiness",basis:"to date",source:"Happiness survey (per sprint). Period view adds the points moved inside the window.",cell:function(c){return{text:c.happier.usage.toFixed(1),rag:F.wq(c.happier.usage,c.thresholds.happier)}},change:function(c,d,period){var v=Math.round(10*F.hy(c,period).happier*HAPPIER_MIX.usage)/10;return{text:wd(c.happier.usage.toFixed(1),sNum1(v)),rag:F.wq(c.happier.usage,c.thresholds.happier)}}},{label:"Happiness at completion",basis:"forecast",emphasis:!0,source:"Derived: average of the three survey scores. Happiness has no separate forecast model, so today's score is the forecast.",cell:function(c,d){return{text:d.happierScore.toFixed(1),rag:F.wq(d.happierScore,c.thresholds.happier)}},change:function(c,d,period){var v=F.hy(c,period).happier;return{text:wd(d.happierScore.toFixed(1),sNum1(v)),rag:F.wq(d.happierScore,c.thresholds.happier)}}},{label:"Agreed target",basis:"agreed",source:"Configuration, RAG threshold for Happier.",cell:function(c){return{text:c.thresholds.happier.green.toFixed(1)}}}]},
...
metrics:[{label:"Actuals to date",basis:"to date",source:"Hour: Oracle x rate. FP: sum of function prices. Period view adds what was booked inside the window.",cell:function(c,d){return{text:F.s7(d.actuals)}},change:function(c,d,period){return{text:wd(F.s7(d.actuals),sMoney(spendIn(c,d,period)))}},rollupChangeCell:function(configs,derived,period){var v=sumBy(configs,function(c,i){return spendIn(c,derived[i],period)});return{text:wd(F.s7(sumBy(derived,function(d){return d.actuals})),sMoney(v))}}},{label:"Estimate to Complete",basis:"forecast",source:"Hour: estimate (complexity x rate x change factor). FP: agreed price after change factor. Period view adds how the remaining estimate moved.",cell:function(c,d){return{text:F.s7(d.etc)}},change:function(c,d,period){var v=eacMove(c,d,period)-spendIn(c,d,period);return{text:wd(F.s7(d.etc),sMoney(v)),rag:dirRag(-v)}}},{label:"Estimate at completion (EAC)",basis:"forecast",emphasis:!0,source:"Derived: Actuals + Estimate to Complete. Period view adds how the forecast moved.",cell:function(c,d){return{text:F.s7(d.eac)}},change:function(c,d,period){var v=eacMove(c,d,period);return{text:wd(F.s7(d.eac),sMoney(v)),rag:dirRag(-v)}},rollupChangeCell:function(configs,derived,period){var v=sumBy(configs,function(c,i){return eacMove(c,derived[i],period)});return{text:wd(F.s7(sumBy(derived,function(d){return d.eac})),sMoney(v)),rag:dirRag(-v)}}},{label:"Budget",basis:"agreed",emphasis:!0,source:"Configuration, baseline budget set by the project owner.",cell:function(c){return{text:F.s7(c.baselineBudget)}}},{label:"Variance at completion",basis:"forecast",emphasis:!0,source:"Derived: Budget minus EAC. Period view adds how that gap moved; with the budget fixed it is the EAC movement with the sign flipped.",cell:function(c,d){return{text:money(d.vacCost)+" · "+pctOf(d.vacCost,c.baselineBudget)+(d.vacCost>=0?" under":" over"),rag:d.vacCost>=0?"good":d.vacCost>-.08*c.baselineBudget?"warn":"bad"}},rollupCell:function(configs,derived){var v=sumBy(derived,function(d){return d.vacCost}),b=sumBy(configs,function(c){return c.baselineBudget});return{text:money(v)+" · "+pctOf(v,b)+(v>=0?" under":" over")}},change:function(c,d,period){var v=-eacMove(c,d,period);return{text:wd(money(d.vacCost),sMoney(v)),rag:d.vacCost>=0?"good":d.vacCost>-.08*c.baselineBudget?"warn":"bad"}},rollupChangeCell:function(configs,derived,period){var v=-sumBy(configs,function(c,i){return eacMove(c,derived[i],period)});return{text:wd(money(sumBy(derived,function(d){return d.vacCost})),sMoney(v))}}},{label:"Cost / FP",basis:"to date",emphasis:!0,source:"FPA sheet (Cost divided by #FP). Period view adds how the rate moved inside the window.",cell:function(c,d){return{text:F.M1(d.costPerFp)+"/FP",rag:F.wq(d.costPerFp,c.thresholds.cheaper)}},change:function(c,d,period){var b=before(c,d,period),v=b?d.costPerFp-b.spend/b.fp:0;return{text:wd(F.M1(d.costPerFp)+"/FP",b?sFull(v):""),rag:F.wq(d.costPerFp,c.thresholds.cheaper)}}},{label:"Output tariff (L3)",basis:"agreed",noRollup:!0,source:"Contract (given, FP-based).",cell:function(c){return{text:"fp"===c.contractType?F.M1(c.outputTariff)+"/FP":"n/a"}}}]},
...
return{text:sDays(h.slackDays),sub:shortDate(prevForecast)+" → "+F.G$(d.forecastDate),rag:dirRag(h.slackDays)}},
rollupCompletion:function(configs,derived){
...
return{text:sDays(delta),rag:dirRag(delta)}},
metrics:[{label:"Delivered FP",basis:"to date",source:"FPA sheet (status = Deployed). Period view adds what was deployed inside the window.",cell:function(c,d){return{text:d.deliveredFp+" FP"}},change:function(c,d,period){return{text:wd(d.deliveredFp+" FP",sNum(F.hy(c,period).deliveredFp||0))}},rollupChangeCell:function(configs,derived,period){return{text:wd(sumBy(derived,function(d){return d.deliveredFp})+" FP",sNum(sumH(configs,period,"deliveredFp")))}}},{label:"Remaining scope",basis:"forecast",source:"FPA sheet (status not Deployed). Period view adds scope added less scope deployed inside the window.",cell:function(c,d){return{text:d.remainingFp+" FP"}},change:function(c,d,period){var h=F.hy(c,period),v=(h.scopeFp||0)-(h.deliveredFp||0);return{text:wd(d.remainingFp+" FP",sNum(v)),rag:dirRag(-v)}},rollupChangeCell:function(configs,derived,period){var v=sumH(configs,period,"scopeFp")-sumH(configs,period,"deliveredFp");return{text:wd(sumBy(derived,function(d){return d.remainingFp})+" FP",sNum(v)),rag:dirRag(-v)}}},{label:"Total scope",basis:"forecast",emphasis:!0,source:"Derived: all counted FP, excluding deleted functions.",cell:function(c,d){return{text:d.scopeEac+" FP"}},change:function(c,d,period){var v=F.hy(c,period).scopeFp||0;return{text:wd(d.scopeEac+" FP",sNum(v)),rag:dirRag(-v)}},rollupChangeCell:function(configs,derived,period){var v=sumH(configs,period,"scopeFp");return{text:wd(sumBy(derived,function(d){return d.scopeEac})+" FP",sNum(v)),rag:dirRag(-v)}}},{label:"Baseline scope target",basis:"agreed",source:"Configuration, baseline set by the project owner.",cell:function(c){return{text:c.baselineScopeFp+" FP"}}},{label:"Scope variance",basis:"forecast",emphasis:!0,source:"Derived: total minus target; less scope is fine. Period view adds how the gap moved.",cell:function(c,d){var g=d.scopeGrowth;if(g<=0)return{text:g+" FP"+" · "+(0===g?"on target":"under target"),rag:"good"};return{text:F.qN(g," FP")+" · "+"scope growth",rag:g>.05*c.baselineScopeFp?"bad":"warn"}},change:function(c,d,period){var g=d.scopeGrowth,v=F.hy(c,period).scopeFp||0;return{text:wd(F.qN(g," FP"),sNum(v)),rag:g<=0?"good":g>.05*c.baselineScopeFp?"bad":"warn"}},rollupChangeCell:function(configs,derived,period){var v=sumH(configs,period,"scopeFp");return{text:wd(F.qN(sumBy(derived,function(d){return d.scopeGrowth})," FP"),sNum(v))}}},{label:"Throughput",basis:"to date",source:"FPA sheet (Deployed FP divided by time). Period view adds how the rate moved inside the window.",cell:function(c,d){return{text:d.throughput+" FP/wk"}},change:function(c,d,period){var b=before(c,d,period);return{text:wd(d.throughput+" FP/wk",b?sNum1(d.throughput-b.fp/b.weeks):"")}}},{label:"Delivery speed (L3)",basis:"to date",source:"FPA sheet (Deployed FP per iteration). Period view adds how the rate moved inside the window.",cell:function(c,d){return{text:d.deliverySpeed+" FP/PI"}},change:function(c,d,period){var b=before(c,d,period);return{text:wd(d.deliverySpeed+" FP/PI",b?sNum1(d.deliverySpeed-2*b.fp/b.weeks):"")}}},{label:"Productivity (L3)",basis:"to date",source:"Oracle hours divided by weekly FP. Period view adds how the rate moved inside the window.",cell:function(c,d){return{text:d.productivity+" h/FP"}},change:function(c,d,period){var b=before(c,d,period);return{text:wd(d.productivity+" h/FP",b?sNum1(d.productivity-b.hours/b.fp):"")}}},{label:"Forecast completion date",basis:"forecast",emphasis:!0,noRollup:!0,source:"Derived: remaining scope divided by the run rate, from today. Period view adds how far the date moved inside the window.",cell:function(c,d){return{text:F.G$(d.forecastDate)}},change:function(c,d,period){var v=F.hy(c,period).slackDays;return{text:wd(F.G$(d.forecastDate),0===v?"":sNum(-v)+" "+unit(v,"day","days")),rag:dirRag(v)}}},{label:"Planned deadline",basis:"agreed",noRollup:!0,source:"Configuration, target deadline set by the project owner.",cell:function(c){return{text:F.G$(c.baselineDeadline)}}},{label:"Forecast vs planned",basis:"forecast",emphasis:!0,source:"Derived: forecast date vs deadline. Period view adds how many days the gap moved inside the window.",cell:function(c,d){return{text:F.qN(d.forecastSlackDays," days")+" · "+pctOf(d.forecastSlackDays,d.plannedDays)+(d.forecastSlackDays>=0?" early":" late")}},rollupCell:function(configs,derived){var v=Math.round(avg(derived.map(function(d){return d.forecastSlackDays}))),pl=Math.round(avg(derived.map(function(d){return d.plannedDays})));return{text:F.qN(v," days")+" · "+pctOf(v,pl)+(v>=0?" early":" late")}},change:function(c,d,period){var v=F.hy(c,period).slackDays;return{text:wd(F.qN(d.forecastSlackDays," days"),sNum(v)),rag:F.wq(d.forecastSlackDays,c.thresholds.faster)}},rollupChangeCell:function(configs,derived,period){var v=Math.round(avgH(configs,period,"slackDays")),t=Math.round(avg(derived.map(function(d){return d.forecastSlackDays})));return{text:wd(F.qN(t," days"),sNum(v))}}},{label:"Descoped FP",basis:"forecast",source:"FPA sheet (change type = Deleted; still charged at the agreed removal factor). No period history in this prototype.",cell:function(c,d){return{text:d.deletedFp+" FP"}}}]}];
...
!collapsed&&group.metrics.map(function(metric){
var read=isPeriod&&metric.change?metric.change:metric.cell;
