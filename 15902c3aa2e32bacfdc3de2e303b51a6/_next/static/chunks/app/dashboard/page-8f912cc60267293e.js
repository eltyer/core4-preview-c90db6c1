...
metrics:[{label:"Contract type",basis:"agreed",noRollup:!0,source:"Configuration, contract-type toggle",cell:function(c){return{text:"fp"===c.contractType?"FP-based":"Hour-based"}}},{label:"FP method",basis:"agreed",noRollup:!0,source:"Configuration, FP counting method",cell:function(c){return{text:c.fpMethod}}},{label:"Lines of Code",basis:"to date",source:"TiCS (interface). In period view, lines added inside the window and the resulting total",cell:function(c){return{text:c.overview.loc.toLocaleString("en-US")}},change:function(c,d,period){var v=F.hy(c,period).locAdded||0;return{text:sg(v)+v.toLocaleString("en-US")+"\u2192"+c.overview.loc.toLocaleString("en-US")}},rollupChangeCell:function(configs,derived,period){var v=sumH(configs,period,"locAdded"),t=sumBy(configs,function(c){return c.overview.loc});return{text:sg(v)+v.toLocaleString("en-US")+"\u2192"+t.toLocaleString("en-US")}}},{label:"Complexity Index",basis:"to date",source:"TiCS (interface)",cell:function(c){return{text:c.overview.complexityIndex.toFixed(1)}}},{label:"# Developers",basis:"to date",source:"Oracle / roster",cell:function(c){return{text:String(c.overview.developers)}},rollupCell:function(configs){return{text:String(sumBy(configs,function(c){return c.overview.developers}))}}},{label:"Total scope",basis:"forecast",emphasis:!0,source:"Derived: sum of all counted FP, excluding deleted functions",cell:function(c,d){return{text:d.totalFp+" FP"}},change:function(c,d,period){var v=F.hy(c,period).scopeFp||0;return{text:sFp(v)+"\u2192"+d.totalFp+" FP",rag:dirRag(-v)}},rollupChangeCell:function(configs,derived,period){var v=sumH(configs,period,"scopeFp"),t=sumBy(derived,function(d){return d.totalFp});return{text:sFp(v)+"\u2192"+t+" FP",rag:dirRag(-v)}}}]},
...
metrics:[{label:"Agreed quality grade",basis:"agreed",source:"Configuration, agreed quality grade for this application",cell:function(c){return{text:c.qualityTarget+" ("+F.gF(c.qualityTarget)+"%)"}}},{label:"Security (ISO 5055)",basis:"to date",emphasis:!0,source:"TiCS (interface). In period view, points moved inside the window and the resulting score",cell:function(c){return{text:pct(c.better.security),rag:F.wq(c.better.security,c.thresholds.better)}},change:function(c,d,period){var now=c.better.security,v=Math.round(F.hy(c,period).better*BETTER_MIX.security);return{text:sPts(v)+" ("+(now-v)+"\u2192"+now+"%)",rag:dirRag(v)}}},{label:"Maintainability (ISO 5055)",basis:"to date",source:"TiCS (interface). In period view, points moved inside the window and the resulting score",cell:function(c){return{text:pct(c.better.maintainability),rag:F.wq(c.better.maintainability,c.thresholds.better)}},change:function(c,d,period){var now=c.better.maintainability,v=Math.round(F.hy(c,period).better*BETTER_MIX.maintainability);return{text:sPts(v)+" ("+(now-v)+"\u2192"+now+"%)",rag:dirRag(v)}}},{label:"Reliability (ISO 5055)",basis:"to date",source:"TiCS (interface). In period view, points moved inside the window and the resulting score",cell:function(c){return{text:pct(c.better.reliability),rag:F.wq(c.better.reliability,c.thresholds.better)}},change:function(c,d,period){var now=c.better.reliability,v=Math.round(F.hy(c,period).better*BETTER_MIX.reliability);return{text:sPts(v)+" ("+(now-v)+"\u2192"+now+"%)",rag:dirRag(v)}}},{label:"Efficiency (ISO 5055)",basis:"to date",source:"TiCS (interface). In period view, points moved inside the window and the resulting score",cell:function(c){return{text:pct(c.better.efficiency),rag:F.wq(c.better.efficiency,c.thresholds.better)}},change:function(c,d,period){var now=c.better.efficiency,v=Math.round(F.hy(c,period).better*BETTER_MIX.efficiency);return{text:sPts(v)+" ("+(now-v)+"\u2192"+now+"%)",rag:dirRag(v)}}},{label:"Functional suitability",basis:"to date",source:"TiCS (interface). In period view, points moved inside the window and the resulting score",cell:function(c){return{text:pct(c.better.functionalSuitability),rag:F.wq(c.better.functionalSuitability,c.thresholds.better)}},change:function(c,d,period){var now=c.better.functionalSuitability,v=Math.round(F.hy(c,period).better*BETTER_MIX.functionalSuitability);return{text:sPts(v)+" ("+(now-v)+"\u2192"+now+"%)",rag:dirRag(v)}}},{label:"MTBF",basis:"to date",source:"Jira, incident tickets (open/close). No period history is held for incident statistics in this prototype",cell:function(c){return{text:c.better.mtbf+" days"}}},{label:"MTTR",basis:"to date",source:"Jira, incident tickets (open/close). No period history is held for incident statistics in this prototype",cell:function(c){return{text:c.better.mttr+" hrs"}}},{label:"Quality at completion",changeLabel:"Quality score",basis:"forecast",emphasis:!0,source:"Derived: average of the five ISO 5055 scores. Quality has no separate forecast model, so today's score is the forecast",cell:function(c,d){var g=F.b9(d.betterScore);return{text:g+" ("+d.betterScore+"%)",rag:F.I(g)>=F.I(c.qualityTarget)?"good":F.I(g)===F.I(c.qualityTarget)-1?"warn":"bad"}},change:function(c,d,period){var v=F.hy(c,period).better;return{text:sPts(v)+" ("+(d.betterScore-v)+"\u2192"+d.betterScore+"%)",rag:dirRag(v)}}}]},
...
metrics:[{label:"Agreed target",basis:"agreed",source:"Configuration, RAG threshold for Happier",cell:function(c){return{text:c.thresholds.happier.green.toFixed(1)}}},{label:"Team happiness",basis:"to date",emphasis:!0,source:"Happiness survey (per sprint). In period view, points moved inside the window and the resulting score",cell:function(c){return{text:c.happier.team.toFixed(1),rag:F.wq(c.happier.team,c.thresholds.happier)}},change:function(c,d,period){var now=c.happier.team,v=Math.round(10*F.hy(c,period).happier*HAPPIER_MIX.team)/10;return{text:sDec(v)+" ("+fixed1(now-v)+"\u2192"+now.toFixed(1)+")",rag:dirRag(v)}}},{label:"Collaboration happiness",basis:"to date",source:"Happiness survey (per sprint). In period view, points moved inside the window and the resulting score",cell:function(c){return{text:c.happier.collaboration.toFixed(1),rag:F.wq(c.happier.collaboration,c.thresholds.happier)}},change:function(c,d,period){var now=c.happier.collaboration,v=Math.round(10*F.hy(c,period).happier*HAPPIER_MIX.collaboration)/10;return{text:sDec(v)+" ("+fixed1(now-v)+"\u2192"+now.toFixed(1)+")",rag:dirRag(v)}}},{label:"Usage happiness",basis:"to date",source:"Happiness survey (per sprint). In period view, points moved inside the window and the resulting score",cell:function(c){return{text:c.happier.usage.toFixed(1),rag:F.wq(c.happier.usage,c.thresholds.happier)}},change:function(c,d,period){var now=c.happier.usage,v=Math.round(10*F.hy(c,period).happier*HAPPIER_MIX.usage)/10;return{text:sDec(v)+" ("+fixed1(now-v)+"\u2192"+now.toFixed(1)+")",rag:dirRag(v)}}},{label:"Happiness at completion",changeLabel:"Happiness score",basis:"forecast",emphasis:!0,source:"Derived: average of the three survey scores. Happiness has no separate forecast model, so today's score is the forecast",cell:function(c,d){return{text:d.happierScore.toFixed(1),rag:F.wq(d.happierScore,c.thresholds.happier)}},change:function(c,d,period){var v=F.hy(c,period).happier;return{text:sDec(v)+" ("+fixed1(d.happierScore-v)+"\u2192"+d.happierScore.toFixed(1)+")",rag:dirRag(v)}}}]},
...
metrics:[{label:"Baseline budget",basis:"agreed",emphasis:!0,source:"Configuration, baseline budget set by the project owner",cell:function(c){return{text:F.s7(c.baselineBudget)}}},{label:"Output tariff (L3)",basis:"agreed",noRollup:!0,source:"Contract (given, FP-based)",cell:function(c){return{text:"fp"===c.contractType?F.M1(c.outputTariff)+"/FP":"n/a"}}},{label:"Actuals to date",changeLabel:"Spent",basis:"to date",source:"Hour: Oracle x rate. FP: sum of function prices. In period view, money booked inside the window against actuals to date",cell:function(c,d){return{text:F.s7(d.actuals)}},change:function(c,d,period){return{text:F.s7(spendIn(c,d,period))+" of "+F.s7(d.actuals)}},rollupChangeCell:function(configs,derived,period){var v=sumBy(configs,function(c,i){return spendIn(c,derived[i],period)}),t=sumBy(derived,function(d){return d.actuals});return{text:F.s7(v)+" of "+F.s7(t)}}},{label:"Cost / FP",basis:"to date",emphasis:!0,source:"FPA sheet (Cost divided by #FP). In period view, money spent in the window per FP deployed in the window, against the rate to date",cell:function(c,d){return{text:F.M1(d.costPerFp)+"/FP",rag:F.wq(d.costPerFp,c.thresholds.cheaper)}},change:function(c,d,period){var fp=F.hy(c,period).deliveredFp||0;if(!fp)return{text:"no FP deployed"};var v=spendIn(c,d,period)/fp;return{text:F.M1(v)+" vs "+F.M1(d.costPerFp)+"/FP",rag:F.wq(v,c.thresholds.cheaper)}}},{label:"Estimate to Complete",basis:"forecast",source:"Hour: estimate (complexity x rate x change factor). FP: agreed price after change factor. In period view, how the remaining estimate moved and where it now stands",cell:function(c,d){return{text:F.s7(d.etc)+" \u00b7 "+("fp"===c.contractType?"agreed":"estimate")}},change:function(c,d,period){var v=eacMove(c,d,period)-spendIn(c,d,period);return{text:sMoney(v)+"\u2192"+F.s7(d.etc),rag:dirRag(-v)}}},{label:"Forecast total spend (EAC)",basis:"forecast",emphasis:!0,source:"Derived: Actuals + Estimate to Complete. In period view, how the forecast moved and where it now stands",cell:function(c,d){return{text:F.s7(d.eac)}},change:function(c,d,period){var v=eacMove(c,d,period);return{text:sMoney(v)+"\u2192"+F.s7(d.eac),rag:dirRag(-v)}},rollupChangeCell:function(configs,derived,period){var v=sumBy(configs,function(c,i){return eacMove(c,derived[i],period)}),t=sumBy(derived,function(d){return d.eac});return{text:sMoney(v)+"\u2192"+F.s7(t),rag:dirRag(-v)}}},{label:"Variance at completion (VAC)",basis:"forecast",emphasis:!0,source:"Derived: Budget minus EAC. In period view, how that gap moved - with the budget fixed this is the EAC movement with the sign flipped",cell:function(c,d){return{text:money(d.vacCost)+" \u00b7 "+pctOf(d.vacCost,c.baselineBudget)+(d.vacCost>=0?" under":" over"),rag:d.vacCost>=0?"good":d.vacCost>-.08*c.baselineBudget?"warn":"bad"}},rollupCell:function(configs,derived){var v=sumBy(derived,function(d){return d.vacCost}),b=sumBy(configs,function(c){return c.baselineBudget});return{text:money(v)+" \u00b7 "+pctOf(v,b)+(v>=0?" under":" over")}},change:function(c,d,period){var v=-eacMove(c,d,period);return{text:sMoney(v)+"\u2192"+money(d.vacCost),rag:dirRag(v)}},rollupChangeCell:function(configs,derived,period){var v=-sumBy(configs,function(c,i){return eacMove(c,derived[i],period)}),t=sumBy(derived,function(d){return d.vacCost});return{text:sMoney(v)+"\u2192"+money(t),rag:dirRag(v)}}}]},
...
metrics:[{label:"Planned deadline",basis:"agreed",noRollup:!0,source:"Configuration, target deadline set by the project owner",cell:function(c){return{text:F.G$(c.baselineDeadline)}}},{label:"Baseline scope target",basis:"agreed",source:"Configuration, baseline set by the project owner",cell:function(c){return{text:c.baselineScopeFp+" FP"}}},{label:"Delivered FP",basis:"to date",source:"FPA sheet (status = Deployed). In period view, FP deployed inside the window against total delivered",cell:function(c,d){return{text:d.deliveredFp+" FP"}},change:function(c,d,period){return{text:(F.hy(c,period).deliveredFp||0)+" of "+d.deliveredFp+" FP"}},rollupChangeCell:function(configs,derived,period){return{text:sumH(configs,period,"deliveredFp")+" of "+sumBy(derived,function(d){return d.deliveredFp})+" FP"}}},{label:"Throughput",basis:"to date",source:"FPA sheet (Deployed FP divided by time). In period view, the rate inside the window against the rate to date",cell:function(c,d){return{text:d.throughput+" FP/wk"}},change:function(c,d,period){var v=Math.round(10*(F.hy(c,period).deliveredFp||0)/weeksIn(period))/10;return{text:v+" vs "+d.throughput+" FP/wk"}}},{label:"Delivery speed (L3)",basis:"to date",source:"FPA sheet (Deployed FP per iteration). In period view, the rate inside the window against the rate to date",cell:function(c,d){return{text:d.deliverySpeed+" FP/PI"}},change:function(c,d,period){var v=Math.round(20*(F.hy(c,period).deliveredFp||0)/weeksIn(period))/10;return{text:v+" vs "+d.deliverySpeed+" FP/PI"}}},{label:"Productivity (L3)",basis:"to date",source:"Oracle hours divided by weekly FP. In period view, hours booked in the window per FP deployed in the window, against the rate to date",cell:function(c,d){return{text:d.productivity+" h/FP"}},change:function(c,d,period){var h=F.hy(c,period),fp=h.deliveredFp||0;if(!fp)return{text:"no FP deployed"};return{text:Math.round(10*(c.hoursToDate*h.spendShare/fp))/10+" vs "+d.productivity+" h/FP"}}},{label:"Forecast completion date",changeLabel:"Forecast date",basis:"forecast",emphasis:!0,noRollup:!0,source:"Derived: remaining scope divided by the run rate, from today. In period view, where the forecast sat at the start of the window and where it sits now",cell:function(c,d){return{text:F.G$(d.forecastDate)}},change:function(c,d,period){var h=F.hy(c,period),prev=shiftDate(c.baselineDeadline,-(d.forecastSlackDays-h.slackDays));return{text:shortDate(prev)+"\u2192"+F.G$(d.forecastDate),rag:dirRag(h.slackDays)}}},{label:"Forecast vs planned",basis:"forecast",emphasis:!0,source:"Derived: forecast date vs deadline. In period view, days the forecast moved inside the window and where the gap now stands",cell:function(c,d){return{text:F.qN(d.forecastSlackDays," days")+" \u00b7 "+pctOf(d.forecastSlackDays,d.plannedDays)+(d.forecastSlackDays>=0?" early":" late")}},rollupCell:function(configs,derived){var v=Math.round(avg(derived.map(function(d){return d.forecastSlackDays}))),pl=Math.round(avg(derived.map(function(d){return d.plannedDays})));return{text:F.qN(v," days")+" \u00b7 "+pctOf(v,pl)+(v>=0?" early":" late")}},change:function(c,d,period){var v=F.hy(c,period).slackDays;return{text:sDays(v)+"\u2192"+F.qN(d.forecastSlackDays," days"),rag:dirRag(v)}},rollupChangeCell:function(configs,derived,period){var v=Math.round(avgH(configs,period,"slackDays")),t=Math.round(avg(derived.map(function(d){return d.forecastSlackDays})));return{text:sDays(v)+"\u2192"+F.qN(t," days"),rag:dirRag(v)}}},{label:"Remaining scope",basis:"forecast",source:"FPA sheet (status not Deployed). In period view, scope added less scope deployed inside the window, and what remains",cell:function(c,d){return{text:d.remainingFp+" FP"}},change:function(c,d,period){var h=F.hy(c,period),v=(h.scopeFp||0)-(h.deliveredFp||0);return{text:sFp(v)+"\u2192"+d.remainingFp+" FP",rag:dirRag(-v)}},rollupChangeCell:function(configs,derived,period){var v=sumH(configs,period,"scopeFp")-sumH(configs,period,"deliveredFp"),t=sumBy(derived,function(d){return d.remainingFp});return{text:sFp(v)+"\u2192"+t+" FP",rag:dirRag(-v)}}},{label:"Descoped FP",basis:"forecast",source:"FPA sheet (change type = Deleted; still charged at the agreed removal factor). No period history is held for descoping in this prototype",cell:function(c,d){return{text:d.deletedFp+" FP"}}},{label:"Total scope",basis:"forecast",emphasis:!0,source:"Derived: sum of all counted FP, excluding deleted functions",cell:function(c,d){return{text:d.scopeEac+" FP"}},change:function(c,d,period){var v=F.hy(c,period).scopeFp||0;return{text:sFp(v)+"\u2192"+d.scopeEac+" FP",rag:dirRag(-v)}},rollupChangeCell:function(configs,derived,period){var v=sumH(configs,period,"scopeFp"),t=sumBy(derived,function(d){return d.scopeEac});return{text:sFp(v)+"\u2192"+t+" FP",rag:dirRag(-v)}}},{label:"Scope variance",basis:"forecast",emphasis:!0,source:"Derived: total minus target (less scope is fine). In period view, how the gap to the scope target moved and where it now stands",cell:function(c,d){var g=d.scopeGrowth;if(g<=0)return{text:g+" FP"+" \u00b7 "+(0===g?"on target":"under target"),rag:"good"};return{text:F.qN(g," FP")+" \u00b7 "+"scope growth",rag:g>.05*c.baselineScopeFp?"bad":"warn"}},change:function(c,d,period){var v=F.hy(c,period).scopeFp||0;return{text:sFp(v)+"\u2192"+F.qN(d.scopeGrowth," FP"),rag:dirRag(-v)}},rollupChangeCell:function(configs,derived,period){var v=sumH(configs,period,"scopeFp"),t=sumBy(derived,function(d){return d.scopeGrowth});return{text:sFp(v)+"\u2192"+F.qN(t," FP"),rag:dirRag(-v)}}}]}];
...
(0,jsx.jsx)("span",{children:props.view==="period"?"Every row shows what moved over "+periodLabel(props.period)+". Rows with no period meaning - agreed baselines, and figures this prototype holds no history for - are hidden here and appear under At completion.":"Every metric is tagged with what it is measured against: agreed at baseline, to date, or forecast at completion. A gap to a target is written in words; a movement over time is written with a sign, and never without the absolute it moved between. Thresholds are owner-configurable per category (Configuration)."})]})}}},function(e){e.O(0,[29,620,971,117,744],function(){return e(e.s=1113)}),_N_E=e.O()}]);